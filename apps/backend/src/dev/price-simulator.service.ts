    ...
    this.simulationInterval = setInterval(() => {
      try {
        ...
        this.pricePublisher?.publishPriceUpdate(tick).catch((error) => {
          this.logger.error('Failed to publish simulated price', error instanceof Error ? error.message : String(error));
        });

        this.logger.debug(`📊 Simulated price: ${randomSymbol} = $${tick.price}`);
      } catch (error) {
        this.logger.error('Price simulation error', error instanceof Error ? error.stack : String(error));
      }
